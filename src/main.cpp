...
#if defined(NDEBUG)
# error "Bitrefillcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe22238aca5d9eb7f9454f74608998b3f10ba3dd69e1a76f2d90cf8ad7b2d092e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitrefillcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bitrefillcoin Signed Message:\n";
...
{
    // Bitrefillcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitrefillcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitrefillcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1507776614, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "The Times 11 October 2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 7 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04be0e297124cd0287bb532705e0d1970f89224006f0b5da549122150796b302566fae5b9be6bcae10cd4088b7b47daddeed3add72e347f12146eef60934589c15") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1507776614;
        ...
        assert(block.hashMerkleRoot == uint256("0xa7c2a412dad9346e20cef1bdb91f13add1ae039a3f2356b36d365f80f06b5d50"));
...
unsigned char pchMessageStart[4] = { 0x9a, 0xc2, 0x0f, 0xa7 }; // Bitrefillcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitrefillcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitrefillcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitrefillcoinMiner(CWallet *pwallet)
{
    printf("BitrefillcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitrefillcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BitrefillcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitrefillcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitrefillcoinMiner, pwallet));
}
